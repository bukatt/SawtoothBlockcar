...
from bc_exceptions import BcException


DISTRIBUTION_NAME = 'sawtooth-bc'
...
def add_show_parser(subparsers, parent_parser):
    parser = subparsers.add_parser(
        'show',
        help='Displays information about a vehicle',
        description='Displays the vehicle info',
        parents=[parent_parser])
...
def add_event_parser(subparsers, parent_parser):
    parser = subparsers.add_parser(
        'vevent',
        help='Vehicle history event',
        description='Sends a transaction to add to vehicle history',
        parents=[parent_parser])
...
def create_parser(prog_name):
    ...
    add_event_parser(subparsers, parent_parser)
...
def do_list(args):
    ...
    client = BcClient(base_url=url, keyfile=None)
    ...
    if game_list is not None:
        ...
        raise BcException("Could not retrieve game listing.")
...
def do_show(args):
    ...
    if data is not None:
        ...
        raise BcException("Game not found: {}".format(name))
...
def do_create(args):
    ...
    client = BcClient(base_url=url, keyfile=keyfile)
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None):
    ...
    if args.command == 'create':
        ...
        raise BcClient("invalid command: {}".format(args.command))
