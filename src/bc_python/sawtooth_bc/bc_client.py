...
class BcClient:
    ...
    def list(self, auth_user=None, auth_password=None):
        bc_prefix = self._get_prefix()

        result = self._send_request(
            "state?address={}".format(bc_prefix),
            auth_user=auth_user,
            auth_password=auth_password)
    ...
    def _get_status(self, batch_id, wait, auth_user=None, auth_password=None):
        try:
            ...
            raise BcException(err) from err

    def _get_prefix(self):
        return _sha512('bc'.encode('utf-8'))[0:6]

    def _get_address(self, vin):
        xo_prefix = self._get_prefix()
        game_address = _sha512(vin.encode('utf-8'))[0:64]
    ...
    def _send_request(self,
        ...
        try:
            ...
            if result.status_code == 404:
                raise BcException("No such game: {}".format(name))

            if not result.ok:
                raise BcException("Error {}: {}".format(
    ...
    def _send_bc_txn(self,
        ...
        payload = ",".join([vin, make, model, events]).encode()

        # Construct the address
        address = self._get_address(vin)
